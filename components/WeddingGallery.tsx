...
export default function WeddingGallery({ images, templateKey, animation = 'reveal-fade' }: WeddingGalleryProps) {
      ...
      <div className="px-2 py-2">
        {first && (
          <div className="relative overflow-hidden mb-3" style={{ height: 'min(62vw, 520px)' }}>
            <img src={first} alt="Wedding photo 1" className="w-full h-full object-cover reveal-on-scroll" data-animate={animation} />
            <div className="absolute inset-0" style={{ background: 'rgba(20,10,5,0.08)' }} />
          </div>
        )}
        {rest.length > 0 && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {rest.map((src, i) => (
              <div key={`${src}-${i + 1}`} className="relative overflow-hidden" style={{ height: 170 }}>
                <img src={src} alt={`Wedding photo ${i + 2}`} className="w-full h-full object-cover reveal-on-scroll" data-animate={animation} />
                <div className="absolute inset-0" style={{ background: 'rgba(20,10,5,0.08)' }} />
              </div>
            ))}
          </div>
